...
const port = 3000;
// --- ADDED/UPDATED CODE ---
// FIX: Use Coolify's FQDN or a fixed local URL for robust internal API calls in production.
const API_BASE_URL = dev 
    ? `http://${hostname}:${port}` 
    : `https://${process.env.COOLIFY_FQDN || hostname}`; 
...
app.prepare().then(() => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('private-message', async (data: MessagePayload) => {
            ...
            try {
                // FIX: Use the stable API_BASE_URL for internal server-to-API calls
                const fetchUrl = `${API_BASE_URL}/api/messages`; 
    ...
    httpServer.listen(port, () => {
        console.log(`🚀 Ready on ${API_BASE_URL}:${port}`);
    }).on('error', (err) => {
