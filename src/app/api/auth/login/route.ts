...
//     response.cookies.set('user_id', user.id.toString(), { // Convert user.id to string
//       httpOnly: false, // For demo, allow JS to read. In prod, set to true.
//       secure: process.env.NODE_ENV === 'production', // Use secure in production
//       maxAge: 60 * 60, // 1 hour for demo
//       path: '/',
...
export async function POST(req: NextRequest) {
  try {
    ...
    response.cookies.set('isAuthenticated', 'true', {
      // For demo, httpOnly: false is okay, but true is recommended for security in production
      httpOnly: false,
    ...
    response.cookies.set('user_id', user.id.toString(), {
      // For demo, httpOnly: false is okay, but true is HIGHLY recommended for security in production
      httpOnly: false,
      secure: process.env.NODE_ENV === 'production',
      maxAge: 60 * 60, // 1 hour
      path: '/',
